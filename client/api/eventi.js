  ...
  return `
    <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
      <div style="background: white; padding: 20px; border-radius: 8px; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #034a5a;">Prenotazione Confermata</h2>
...
function createBroadcastTemplate(message, titoloEvento) {
  return `
    <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
      <div style="background: white; padding: 20px; border-radius: 8px; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #034a5a;">Avviso: ${titoloEvento}</h2>
  ...
  try {
    ...
    if (req.method === "GET") {
      if (action === "image" && id) {
        ...
        let buffer;
        if (Buffer.isBuffer(row.immagine_blob)) buffer = row.immagine_blob;
        ...
        else buffer = Buffer.from(new Uint8Array(row.immagine_blob));
      ...
      if (!section && !action) {
        const result = await client.execute(`
            ...
            length(immagine_blob) as blob_size FROM eventi ORDER BY data_evento DESC
        `);
      ...
      if (action === "single" && id) {
        const eventoResult = await client.execute({
          sql: `SELECT id, titolo, descrizione, data_evento, orario, link_esterno, immagine_url, immagine_tipo, visibile, num_max, length(immagine_blob) as blob_size FROM eventi WHERE id = ?`,
          args: [id],
        });
    ...
    if (req.method === "POST") {
      ...
      if (section === "prenotazioni") {
        ...
        const result = await client.execute({
          sql: `INSERT INTO prenotazioni_eventi (evento_id, nome, cognome, email, num_partecipanti, note, data_prenotazione) VALUES (?, ?, ?, ?, ?, ?, datetime('now'))`,
          args: [
            evento_id,
            nome,
            cognome,
      ...
      const result = await client.execute({
        sql: `INSERT INTO eventi (titolo, descrizione, data_evento, orario, link_esterno, immagine_url, visibile, num_max) VALUES (?, ?, ?, ?, ?, ?, 0, ?)`,
        args: [
          titolo,
          descrizione || "",
          data_evento,
    ...
    if (req.method === "PUT") {
      ...
      await client.execute({
        sql: `UPDATE eventi SET titolo=?, descrizione=?, data_evento=?, orario=?, link_esterno=?, immagine_url=?, num_max=? WHERE id=?`,
        args: [
          titolo,
          descrizione || "",
          data_evento,
    ...
    return res
      .status(500)
      .json({ success: false, error: "Errore server: " + error.message });
  }
}
